...
fun AlgorithmScreen(
    ...
    Column(
        ...
        AlgorithmSection(title = "优秀兴趣曲线") {
            ...
            BulletPoint("Rising Peaks (波峰与波�?：交替上升的节奏，每次高潮都比上一次更强烈�?)
        ...
        AlgorithmSection(title = "排序算法") {
            Text(
                text = "LiveSort 的核心是一个贪心优化算法。对于播放列表中的每一个位置，算法会计算剩余所有候选歌曲的“代价函�?(Cost Function)”，并选择代价最小的歌曲填入该位置。\n\n代价函数综合考虑了三个权重因素：",
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
                lineHeight = 22.sp
            )
            Spacer(modifier = Modifier.height(8.dp))
            NumberedPoint("1", "情绪贴合�?(最高权�?：候选歌曲的情绪得分与“兴趣曲线”当前位置理想得分的绝对差值�?)
            NumberedPoint("2", "BPM 衔接度：上一首歌结尾�?BPM 与候选歌曲开头的 BPM 的比例。完美衔接（1:1）或倍速衔接（1:2，例�?60 BPM �?120 BPM）会获得极低的惩罚代价�?)
